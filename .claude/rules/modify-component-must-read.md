...
- **JIT CUDA kernel**（`rapid_llm/kernels/csrc/**`，JIT 编译的原生 CUDA 算子）→ [`add-jit-kernel`](../../.claude/skills/add-jit-kernel/SKILL.md)
- **AOT 原生扩展**（`csrc/**`，CUTLASS 级依赖 / 随 wheel 预编译的 CUDA/C++ 算子）→ [`add-sgl-kernel`](../../.claude/skills/add-sgl-kernel/SKILL.md)
...
- **引擎级基准与对外数字**（`benchmarks/engine/**`、`benchmarks/suites/**`、性能文档结论）→ [`model-benchmark-and-report`](../../.claude/skills/model-benchmark-and-report/SKILL.md)
